...
> A facility-wide spatial reference system for describing observation, movement, camera coverage, zone context, incident locations, patrol tasking, door relationships, and recovery through fixed floor-specific coordinates.
...
| Status | Conceptual |
| Scope | Facility-wide observation, movement tracking, camera coordination, incident response, patrol tasking, door relationships, and recovery |
| Related Areas | Surveillance, Incident Response, Zone Model, Master Watcher, Watcher Operations, Incident Maneuver and Door Control, Human Factors, Recovery |
...
- door and access-point correlation
...
The Facility Chessboard Coordinate Layer defines the spatial reference used by multiple operational subsystems.

It provides:

- operational floor identity
- fixed grid location
- local-sector precision
- camera location
- camera field-of-view relationships
- zone and sub-zone relationships
...
Those attributes are interpreted by the wider architecture.
...
The call sign identifies the assigned operational guard.

The coordinate identifies the task location.
...
Doors and passage points should be linked to coordinates.

Each door record may include:

- door identifier
- coordinate
- adjacent coordinate
- connected zones
- permitted passage direction
...
The Incident Maneuver and Door Control Model defines how the door may be used during an incident.

A coordinate should not imply that a door is:
...
Those states must be provided by the relevant door and access-control systems.

---

## Route and Passage Mapping
...
The route should be interpreted together with:
...
- movement during degraded operations
...
- assigned patrol guards
- related door and passage points
...
Watcher Operators should be able to:
...
- update a task
- request or manage a maneuver action
- cancel or escalate an assignment
- hand off responsibility
- record notes
- support recovery reconciliation

The Watcher Operator should see the wider incident context even when the guard sees only:
...
- treating commanded door states as verified physical states
...
- door and passage behavior follows approved emergency procedures
- recovery references remain tied to actual facility locations
...
- direct Watcher Operator confirmation
...
The system should not silently continue an outdated assignment or route when confidence in the task state is lost.
...
4. The Watcher Operator reviews cameras and surrounding zone context.
...
14. The Watcher Operator confirms arrival and observation.
15. Related door and passage states are reviewed if maneuver control is active.
16. The task is completed, updated, cancelled, or escalated.
17. The complete event history remains available for review and recovery reconciliation.

---

## Recovery and Reconciliation

The coordinate layer supports recovery by preserving relationships between:

- incident location
- subject or asset position
- guard assignment
- camera observations
...
After an incident or degraded state, the system may be used to:

- reconcile guard locations
- verify last-known positions
- review movement trails
- validate temporary route changes
- compare camera and coordinate data
- confirm door and passage restoration
- identify unresolved positions
- support controlled re-entry
- preserve the final operational picture
...
- doors and passage points are linked to coordinates
...
- door and passage relationships are visible where required
- radio fallback exists
- device failure has defined handling
- life-safety references remain clear
- recovery reconciliation is supported
...
Coordinates support incident localization, guard tasking, observation, handoff, maneuver, containment, and recovery.
...
The Master Watcher Operator provides central spatial coordination, while Watcher Operators and patrol displays support detailed execution.
...
The maneuver layer uses coordinate relationships to manage doors, passages, and controlled routes.

### Recovery Model

Coordinate history supports reconciliation, controlled re-entry, and restoration of operational legitimacy.
...
- reliable door and passage data
- regular testing
- disciplined review
...
> **A precise shared coordinate system allows authorized personnel to understand the facility quickly, coordinate movement discreetly, shape controlled routes, and reduce the need to expose descriptive architectural information over shared communication channels.**
...
> **The coordinate identifies where a door or passage exists; the maneuver model defines how it may be used.**

> **The control room sees the whole facility; the guard receives the minimum actionable task.**

> **Last observed, last confirmed, and currently verified are separate states.**
